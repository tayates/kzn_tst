### Mixture analysis, tuberculin skin test survey KZN 2013
...
# I am fairly sure I borrowed some code to make this work
# For a much simpler version, see sensitivity analysis script
# The core of this is the normalmixEM function
# Constraints: two components with means that differ and are of positive sign

mixmdl1 <- normalmixEM(x, k = 2, mean.constr = c("a", "b"))
summary(mixmdl1)

plot.mixEM2 <- function (x, whichplots = 1, loglik = 1 %in% whichplots, density = 2 %in% 
...
plot.mixEM2(mixmdl1,which=2, axes=FALSE, breaks=seq(0,24,by=1),ylim=c(0,.12), xlab2 = "Induration in mm", main2="")
lines(density(x), lty=2, lwd=2)
axis(1, 0:24, cex.axis=1.2)
axis(2)

# If you run this code repeatedly, the results of the mixture model are unstable
# This is because a seed is not specified
# The code generates one of two results, as discussed in the paper
